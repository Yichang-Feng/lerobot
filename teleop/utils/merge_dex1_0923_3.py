#!/usr/bin/env python3
"""
Merge g1_pick_put_dex1_0923_3 into g1_pick_put_dex1, sort/reindex, and update prompts.
"""
...
def main():
    ...
    src_dir = base_data_dir / "g1_pick_put_dex1_0923_3"
    ...
    assert len(existing_eps) == 85, f"Expected 85 episodes in {dest_dir.name}, found {len(existing_eps)}"
    assert existing_eps[0].name == "episode_0000"
    assert existing_eps[-1].name == "episode_0084"

    # New episodes in g1_pick_put_dex1_0923_3
    new_eps = sorted([
    ...
    assert len(all_merged_eps) == 143, f"Expected 143 episodes, got {len(all_merged_eps)}"
    assert all_merged_eps[0].name == "episode_0000"
    assert all_merged_eps[-1].name == "episode_0142"
    ...
    print(f"  Total episodes:   {len(all_merged_eps)} (episode_0000 - episode_0142)")
